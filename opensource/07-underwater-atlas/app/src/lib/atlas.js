...
/* ═══════════════════════════════════════════════════════════════════════
   Anki Abyss — the knowledge-atlas engine re-rendered as a bright lagoon.

   What changed from the original atlas:
   · Graph data is rebuildable module state. loadPdfGraph(NODES, EDGES,
     CLUSTERS, meta) assembles nodes/edges/clusters/anchors from scratch;
     the app then disposes the atlas and calls createAtlas again, and the
     whole scene — GPU buffers included — is rebuilt on the new graph.
   · One theme only: "lagoon". No theme switch anywhere.
   · Scene: turquoise water dome with sun-glare at the zenith, swaying god
     rays, a caustic-shimmer sand disc below the graph, rising bubbles.
   · Nodes are solid jewel-tone beads (NormalBlending, never additive);
     edges keep cluster hues with a glinting pulse traveling source→target.
   ═══════════════════════════════════════════════════════════════════════ */
...
export function loadPdfGraph(NODES, EDGES, CLUSTERS, meta = {}) {
  ...
  const nodes = NODES.map(([name, cid, w, desc], i) => ({
...
/* Boot with the default reef so the page works before any import. */
loadPdfGraph(DEFAULT_DECK.nodes, DEFAULT_DECK.edges, DEFAULT_DECK.clusters,
  { id: DEFAULT_DECK.id, name: DEFAULT_DECK.name });
...
export function buildCards(graph = G) {
  const cards = graph.nodes.map(n => ({
    front: `What is ${n.name}?`,
    back: n.desc,
    tags: `anki_abyss ${graph.meta.id} ${n.cid}`,
  }));
  for (const e of graph.edges) {
    if (e.kind !== 'pre') continue;
    cards.push({
      front: `What should you understand before ${e.t.name}?`,
      back: `${e.s.name} — ${e.s.desc}`,
      tags: `anki_abyss ${graph.meta.id} prerequisite`,
    });
  }
  return cards;
}

export function toAnkiText(cards, deckName = G.meta.name) {
  const header = [
    '#separator:tab',
    '#html:false',
    `#deck:Anki Abyss::${deckName}`,
    '#tags column:3',
    '#notetype:Basic',
  ];
  const lines = cards.map(c =>
    [c.front, c.back, c.tags].map(f => String(f).replace(/[\t\r\n]+/g, ' ')).join('\t'));
...
/* Lagoon palette (scene-side; CSS owns the chrome). */
const WATER = 0x2ea9bf;
...
export function createAtlas({ els, emit }) {
  ...
  if (!ok) { emit.gate(true); return { dispose() { } } }
  {
    ...
    /* ── Water dome: turquoise lagoon with sun-glare at the zenith ──────
       A big BackSide sphere; custom gradient instead of scene.background
       so the sun can live inside the water column. */
    const domeMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: /* glsl */`
        ...
        void main(){
          ...
          float h = d.y;
          vec3 deep = vec3(0.05, 0.38, 0.50);
          vec3 mid  = vec3(0.16, 0.66, 0.74);
          vec3 top  = vec3(0.62, 0.91, 0.95);
          vec3 col = mix(deep, mid, smoothstep(-1.0, -0.05, h));
          col = mix(col, top, smoothstep(0.0, 0.85, h));
          // Sun-glare: tight hot core plus a broad soft halo at the zenith.
          vec3 sun = normalize(vec3(0.22, 1.0, 0.14));
          float sd = max(dot(d, sun), 0.0);
          col += pow(sd, 90.0) * vec3(1.0, 0.97, 0.84) * 0.95;
          col += pow(sd, 6.0) * vec3(0.85, 1.0, 0.95) * 0.22;
          // Gentle caustic shimmer drifting across the water column.
          float ca = sin(d.x * 24.0 + uTime * 0.35) * sin(d.z * 21.0 - uTime * 0.27);
          col += ca * 0.018 * smoothstep(-0.3, 0.6, h);
    ...
    /* ── Sandy seafloor: a pale disc below the graph, with an animated
    ...
    const sandMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: /* glsl */`
        ...
        void main(){
          float r = length(vUv);
          vec3 sand = vec3(0.93, 0.88, 0.74);
          // Faint ripple bands, like wind-combed sand.
          float rip = sin(vUv.x * 34.0 + sin(vUv.y * 9.0) * 2.2) * 0.5 + 0.5;
          sand *= 0.96 + rip * 0.05;
          // Caustics: two interfering wave sets, sharpened into bright nets.
          ...
          sand += ca * vec3(1.0, 0.99, 0.90) * 0.34;
          // Darken slightly under the graph: the reef casts a soft shadow.
          sand *= 1.0 - smoothstep(0.5, 0.0, r) * 0.10;
    ...
    const RAY_FRAG = /* glsl */`
      ...
      void main(){
        ...
        gl_FragColor = vec4(vec3(1.0, 0.99, 0.90), a * 0.15);
      }`;
    ...
    const bubMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: /* glsl */`
        ...
        void main(){
          ...
          float body = smoothstep(1.0, 0.2, d) * 0.16;
          float hl = smoothstep(0.34, 0.0, length(gl_PointCoord - vec2(0.36, 0.34)));
          gl_FragColor = vec4(vec3(1.0), (ring * 0.55 + body + hl * 0.5) * vAlpha);
        }`,
    });
    ...
    /* ── Nodes: solid jewel-tone beads, NormalBlending — never additive.
          Two layers share one geometry: a faint soft halo for presence,
          and a shaded bead with a specular glint on top. */
    ...
    const glowMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: /* glsl */`
        ...
        void main(){
          ...
          if (d > 1.0) discard;
          // A faint tinted halo, like the bead staining the water around it.
          float halo = pow(1.0 - d, 3.0);
          gl_FragColor = vec4(vColor * 0.72, halo * 0.17 * vAlpha);
        }`,
    ...
    const coreMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: /* glsl */`
        ...
        void main(){
          float d = length(gl_PointCoord - 0.5) * 2.0;
          if (d > 1.0) discard;
          // Solid bead: the rim darkens as self-shadowing, which gives the
          // bead volume against bright water.
          float disc = smoothstep(1.0, 0.86, d);
          float edge = smoothstep(0.40, 1.0, d);
          vec3 c = mix(vColor, vColor * 0.55, edge);
          // Specular glint up-left, as if lit by the zenith sun.
          vec2 sp = gl_PointCoord - vec2(0.36, 0.34);
          float spec = exp(-dot(sp, sp) * 22.0);
          c += spec * 0.55;
          gl_FragColor = vec4(c, disc * 0.97 * vAlpha);
        }`,
      transparent: true, blending: THREE.NormalBlending, depthWrite: false,
    });
    ...
    const lineMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: /* glsl */`
        ...
        void main(){
          ...
          gl_FragColor = vec4(mix(base, glint, pulse), vAlpha * (0.36 + 0.56 * pulse));
        }`,
      transparent: true, blending: THREE.NormalBlending, depthWrite: false,
    });
